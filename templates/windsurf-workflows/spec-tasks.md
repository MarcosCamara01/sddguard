...
First action: invoke `sddx-workflow snapshot <spec>` to capture the approved spec state. If the CLI is unavailable, warn the user that no snapshot was taken and proceed — do not abort, but make the missing safety net explicit.
